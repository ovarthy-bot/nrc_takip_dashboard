...
const App = {
    state: {
        ...
        lastImportTime: null,
        aircraftImportTimes: {} // aircraftName -> ISO timestamp
    },
    ...
    loadData: async function () {
        ...
        if (metadata) {
            ...
            this.state.aircraftImportTimes = metadata.aircraftImportTimes || {};
        }
    ...
    finalizeProcess: async function (dataMap, headers, importedAircraftByWO) {
        ...
        await Storage.saveMetadata({
            ...
            aircraftImportTimes: this.state.aircraftImportTimes
        });

        this.calculateStats();
        this.populateAircraftFilter();
        // Re-show timestamp if an aircraft is currently selected
        const selectedAircraft = document.getElementById('aircraft-filter').value;
        if (selectedAircraft) this.updateAircraftTimestamp(selectedAircraft);
        this.render();
    ...
    deleteSelectedAircraftData: async function () {
        ...
        // Remove per-aircraft timestamp
        delete this.state.aircraftImportTimes[aircraft];
        ...
        // Clear filter and hide button + timestamp
    ...
    updateAircraftTimestamp: function (aircraftName) {
        const el = document.getElementById('aircraft-import-time');
        if (!el) return;
        if (aircraftName && this.state.aircraftImportTimes[aircraftName]) {
            const date = new Date(this.state.aircraftImportTimes[aircraftName]);
    ...
    deleteAllDashboardData: async function () {
        ...
        this.state.aircraftImportTimes = {};
        // Hide aircraft timestamp
        const tsEl = document.getElementById('aircraft-import-time');
        if (tsEl) { tsEl.textContent = ''; tsEl.classList.add('hidden'); }
        // Hide delete aircraft button
        document.getElementById('delete-aircraft-data-btn').classList.add('hidden');
